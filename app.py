...
with st.expander("📖 使い方", expanded=False):
    st.markdown(
        """
        1. **最初に「事前登録」を済ませます**（企業ごとに1回だけ）。
           弥生会計から **補助科目一覧・勘定科目一覧・摘要科目一覧** の3つをPDF出力し、
           下の「事前登録①②③」にアップロードします。科目・補助科目・摘要は会計事務所・
           お客様ごとに違うため、これを入れることでその会社専用の振り分けになります
        2. **書類タイプを選択** します（領収書／レシート・通帳・カード明細・給与台帳・
           売上（売掛表）・売上請求書・仕入請求書・買掛表 など）
        3. **ファイルをアップロード** します（PDF・PNG・JPG・XLSX・CSV。複数まとめて選択できます）
           - 買掛表は **ExcelやCSVのままアップロード** します（OCR不要でそのまま解析）。他の書類は写真・PDFでOKです
        4. **「変換を開始」** をクリックします（読み取りに数十秒かかることがあります）
        5. **「✏️ 仕訳の編集」** タブで内容を確認します
...
# --- 事前登録のチェックリスト（3つ揃うまで取り込み欄の上に案内を出す） ---
_setup_items = [
    ("補助科目", len(_master), "事前登録①"),
    ("勘定科目", len(_acct_master), "事前登録②"),
    ("摘要辞書", len(_desc_dict), "事前登録③"),
]
if any(count == 0 for _name, count, _where in _setup_items):
    _status = " ／ ".join(
        f"{name} ✅ {count}件" if count else f"{name} ⬜ 未登録"
        for name, count, _where in _setup_items
    )
    _missing = "・".join(where for _name, count, where in _setup_items if count == 0)
    st.info(
        f"**「{client}」の事前登録:** {_status}\n\n"
        f"科目・補助科目・摘要は会社ごとに違うため、先に弥生のPDFを「{_missing}」に登録すると、"
        "その会社専用の振り分けになり精度が上がります（未登録でも取り込みはできます）。"
    )
